...
class ChatViewController: UIViewController, ChatContextMenu {
    ...
    override func viewDidLoad() {
        ...
        infoItem.tintColor = #colorLiteral(red: 0.8031229377, green: 0.691909194, blue: 0.2029924691, alpha: 1)
...
//        }
    ...
    private func loadMessages(with skip: Int = 0) {
        SVProgressHUD.show()
        chatManager.messages(withID: dialogID, skip: skip, limit: ChatManagerConstant.messagesLimitPerDialog,
    ...
    private func setupInputToolbar() {
        ...
        let accessoryImage = UIImage(named: "ic_attachment_Image")
        let normalImage = accessoryImage?.imageMasked(color: #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1))
        let highlightedImage = accessoryImage?.imageMasked(color: #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1))
        ...
        let normalSendImage = accessorySendImage?.imageMasked(color: #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1))
        let highlightedSendImage = accessorySendImage?.imageMasked(color: #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1))
    ...
    private func cancelUploadFile() {
        ...
        let cancelAction = UIAlertAction(title: "SA_STR_CANCEL".localized, style: .default) { (action) in
...
//        present(actionsMenuVC, animated: false)
    ...
    private func didGroupLeaveTapped() {
        if Reachability.instance.networkConnectionStatus() == .notConnection {
            showAlertView(LoginConstant.checkInternet, message: LoginConstant.checkInternetMessage)
            SVProgressHUD.dismiss()
            return
        ...
        let alertController = UIAlertController(title: "SA_STR_WARNING".localized,
                                                message: "SA_STR_LEAVE_GROUP_MESSAGE".localized,
                                                preferredStyle: .alert)
        
        let cancelAction = UIAlertAction(title: "SA_STR_CANCEL".localized, style: .default, handler: nil)
        
        let leaveAction = UIAlertAction(title: "SA_STR_LEAVE_GROUP_ButtonTitle".localized, style: .default) { (action:UIAlertAction) in
    ...
    private func send(withMessageText text: String) {
        ...
        message.text = text
        message.senderID = currentUserID
        message.dialogID = dialogID //reciever Id
        message.deliveredIDs = [(NSNumber(value: currentUserID))]
        message.readIDs = [(NSNumber(value: currentUserID))]
        message.dateSent = Date()
        message.customParameters["save_to_history"] = true
    ...
    func sendPushNotificationSetup(pushMessage: QBChatMessage, PushDialog: QBChatDialog) {
        ...
        if PushDialog.recipientID > 0 {
            QBRequest.sendPush(withText: pushMessage.text ?? "", toUsers: "\(PushDialog.recipientID)") {
                (response, event) in
...
extension ChatViewController: ChatCellDelegate {
    ...
    private func handleNotSentMessage(_ message: QBChatMessage,
        ...
        let delete = UIAlertAction(title: "SA_STR_DELETE_MESSAGE".localized, style: .default) { (action) in
    ...
    func chatCell(_ cell: ChatCell, didTapOn result: NSTextCheckingResult) {
        ...
        case NSTextCheckingResult.CheckingType.phoneNumber:
            ...
            let openAction = UIAlertAction(title: "SA_STR_CALL".localized, style: .default) { (action) in
...
extension ChatViewController: notificationPopUpClickManagerDelegate{
